...
result_path = "household/household_result_mlp" # saving the processing of training phase and images ploted
num_features = 1
input_width = 168
...
class MLP_BASE(tf.keras.Model):
    def __init__(self,
        ...
        self.layer1 = layers.Dense(units=32, input_dim=input_width, activation='relu')
...
# Get dataset
dataset = Dataset(dataset_name="HOUSEHOLD")
raw_data = dataset.dataloader.data_by_hour
# print(type(raw_data))
...
for output_width in [1, 12, 24, 36, 48, 60, 72, 84]:
    # Search model
    tsf = TSF_Data(data=raw_data['Global_active_power'],
